...
static void MP4_FreeBox_avcC( MP4_Box_t *p_box )
{
	...
	if ( p_avcC->i_sps > 0 ) {
		FREENULL( p_avcC->i_sps_length );
	}
	if ( p_avcC->i_pps > 0 ) {
		FREENULL( p_avcC->pps );
	}
	if ( p_avcC->i_pps > 0 ) {
		FREENULL( p_avcC->i_pps_length );
	}
}
