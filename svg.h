/* public domain Simple, Minimalistic, svg library
...
svg* svg_create(int width, int height)
{
	...
	if (psvg != NULL) {
		*psvg = (svg) {
			.svg = NULL, .width = width, .height = height, .finalized = false
		};
...
void svg_scatter(svg* psvg, double *x, double *y, int n, int16_t *t, int tn)
{
	int w = 360;
	int h = 350;
	int wx = 40;
	...
	double ax = 1.0/(xmax-xmin)*w;
	double ay = 1.0/(ymax-ymin)*h;

	char s[256];
//	svg *psvg = svg_create(512, 512);
//	if (!psvg) return;
//	svg_rectangle(psvg, w, h, wx, wy, "#C0C0FF", "black", 2, 0, 0); // 枠線
	...
	for (int c=0; c<256; c++) {
		...
		int sy = (int)(wy+(h/256.0)*(256-c));
		svg_rectangle(psvg, 20, (int)(h/256.0*2), wx+w+30, sy, s, "#696969", 0, 0, 0);
	}
	for (int i=0; i<tn; i++) {
		sprintf(s, "%d", i);
		svg_text(psvg, wx+w+55, (int)(wy+((float)h/tn)*(tn-i)), "sans-serif", 8, "#000000", "#000000", s);
	}
...
/*	svg_line(psvg, "#000000", 2, wx, 50, 400, 50); // 横線
	svg_line(psvg, "#000000", 2, wx, 100, 400, 100);
	svg_line(psvg, "#000000", 2, wx, 150, 400, 150);
	svg_line(psvg, "#000000", 2, wx, 200, 400, 200);
	svg_line(psvg, "#000000", 2, wx, 250, 400, 250);
	svg_line(psvg, "#000000", 2, wx, 300, 400, 300);*/
