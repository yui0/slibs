...
float rabdist(vec3 p)
{
    ...
    float iTime2=iTime*2.4;

    p.y+=2.3;
    p.xy*=rotmat(cos(iTime2+1.)*.04);
    p.y-=2.3;

    vec3 op=p;

    vec3 p2=p;
    p2.xy*=rotmat(cos(iTime2)*.1);

    vec3 p3=p;
    p3.xy*=rotmat(cos(iTime2-.0-length(p)/2.)*.13);
    ...
    float ld=-.75;
    p4.y-=ld;
    p4.yz*=rotmat(cos(iTime*2.+3.1415926/1.*0.)*.1);
    p4.y+=ld;
    p4.y-=max(0.,cos(iTime*2.+3.1415926/2.))*.1;
    d=smin(d,distance(vec3(.3,clamp(p4.y,-2.6,-2.),0.),p4.xyz)-.3,.1);
    p4=op;
    p4.y-=ld;
    p4.yz*=rotmat(cos(iTime*2.+3.1415926/1.*1.)*.1);
    p4.y+=ld;
    p4.y-=max(0.,cos(iTime*2.+3.1415926/2.+3.1415926/1.*1.))*.1;
    d=smin(d,distance(vec3(.3,clamp(p4.y,-2.6,-2.),0.),p4.xyz*vec3(-1,1,1))-.3,.1);   
    
    // Belly
    d=smin(d,distance(vec3(0.,-1.5,-.2),p)-.5+cos(iTime*3.)*.03,.4);   
    ...
    float iTime4=iTime/8.;
    float gg=smoothstep(0.,1.,clamp((min(fract(iTime4),1.-fract(iTime4))-.25)*64.,0.,1.));
